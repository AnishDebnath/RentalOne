...
router.get('/rentals/:id', roleMiddleware(['admin', 'manager', 'staff']), validate(rentalIdParamSchema, 'params'), async (req: Request, res: Response) => {
  ...
  let rentalQuery = supabase.from('rentals').select('id, rental_no, user_id, pickup_date, event_date, total_amount, status, products, handover_proof_url, released_to_representative_name, returned_by_representative_name, assistant_crew_count, released_at, received_at, created_at');

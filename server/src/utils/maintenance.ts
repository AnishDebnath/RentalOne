...
export const startMaintenanceWorker = (intervalMs = 60 * 60 * 1000) => {
  ...
  maintenanceInterval = setInterval(runRentalMaintenance, intervalMs);

  console.log(`[Maintenance] Background worker started. Checks every ${intervalMs / (60 * 1000)} minutes.`);
};

/**
 * Stop the maintenance background worker (for graceful shutdown)
 */
export const stopMaintenanceWorker = () => {
  if (maintenanceInterval) {
    clearInterval(maintenanceInterval);
    maintenanceInterval = null;
    console.log('[Maintenance] Background worker stopped.');
  }
};

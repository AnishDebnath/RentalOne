...
const UserCard = ({ users }: UserCardProps) => {
  ...
  useEffect(() => {
    ...
    const targetTicks = Math.min(users.length, 25);
    const batchSize = Math.ceil(users.length / targetTicks);
    const delay = Math.max(60, Math.min(250, Math.floor(2000 / targetTicks)));
    const timer = setInterval(() => {
      setVisibleCount(prev => {
        const next = Math.min(prev + batchSize, users.length);
  ...
  const TableComponent = users.length > 150 ? VirtualizedDataTable : DataTable;

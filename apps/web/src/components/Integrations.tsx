      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-3xl mx-auto text-center mb-20">
          ...
          <h2 className="font-display text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 mb-4">
            Connect your favorite <span className="bg-gradient-to-r from-primary via-secondary to-primary bg-clip-text text-transparent font-display">Tools Semantics</span>
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex items-center justify-center relative min-h-[380px] md:min-h-[440px] bg-slate-50/50 rounded-3xl p-6 border border-slate-100/80">
            ...
            {integrationNodes.map((node) => {
              ...
              return (
                <div
                  ...
                  className={`lg:absolute ${node.pos} z-10 m-3 lg:m-0 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all duration-300 ${
                    isActive ? 'scale-110' : 'hover:scale-105'
                  }`}
                >
                  <div className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center relative shadow-sm transition-all ${
                    isActive 
                    ...
                    {isConnected && (
                      <span className="absolute top-[-2px] right-[-2px] w-3 h-3 bg-emerald-500 border-2 border-white rounded-full" />
                    )}
                  </div>
          ...
          <div className="lg:col-span-5 bg-slate-50 rounded-2xl border border-slate-100 p-6 md:p-8 flex flex-col justify-between min-h-[360px] text-left">
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      connectionStates[selectedNodeInfo.id] ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'
                    }`}>
            ...
            <div className="border-t border-slate-200/60 pt-6 mt-6 flex flex-col sm:flex-row gap-3 items-center justify-between">
              ...
              {selectedNodeInfo && (
                <button
                  ...
                  className={`w-full sm:w-auto text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-xs flex items-center justify-center gap-2 ${

  ...
  const logos = [
    {
      name: 'StudioPro',
      svg: (
        <svg className="h-8 text-slate-400 group-hover:text-primary transition-colors" viewBox="0 0 120 28" fill="currentColor">
    ...
    },
    {
      name: 'EventWorks',
      svg: (
        <svg className="h-8 text-slate-400 group-hover:text-primary transition-colors" viewBox="0 0 130 28" fill="currentColor">
        ...
        <div className="relative w-full overflow-hidden select-none">
          <div className="flex overflow-hidden w-full">
            <div className="animate-marquee flex gap-12 md:gap-16 lg:gap-20 items-center">
              {logos.map((logo) => (
                <div key={logo.name} className="group cursor-pointer shrink-0">
                  {logo.svg}
                </div>
              ))}
              {/* Duplicate for infinite effect */}
              {logos.map((logo) => (
                <div key={`dup-${logo.name}`} className="group cursor-pointer shrink-0">
                  {logo.svg}
                </div>
              ))}
            </div>
